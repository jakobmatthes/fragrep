...
#include "aln_util.h"
	...
	//#include "pair_mat.h"
	/*@unused@*/
	static char rcsid[] = "$Id: aln_util.cpp,v 1.3 2007/04/25 11:58:05 axel Exp $";

	static const char Law_and_Order[] = "_ACGUTXKI";

	int read_alignment(FILE *faln, char *AlignedSeqs[], char *names[]) {
		char *line, name[100]="";
		
		if ((line=get_line(faln)) == NULL) {
			fprintf(stderr, "Empty alignment file.\n"); return 0;
		}
		
		if (strncmp(line,"CLUSTAL", 7) ==0) {
			free(line);
			return read_clustal(faln, AlignedSeqs, names);
		} else if (strncmp(line,"# STOCKHOLM", 11) ==0) {
			free(line);
			return read_stockholm(faln, AlignedSeqs, names);
		} else {
			fprintf(stderr, "This doesn't look like a CLUSTAL or a STOCKHOLM file, sorry\n");
		}
		free(line);
	}
...
#define MAX_NUM_NAMES    500
	...
	int read_stockholm(FILE *clust, char *AlignedSeqs[], char *names[]) {
		char *line, name[100]="", *seq;
		int  n, nn=0, num_seq = 0;
		...
		while (line!=NULL)
		{
			...
			if (((n=strlen(line))<4) || isspace((int)line[0])
				|| line[0]=='#' && strncmp(line,"#=FRAGREP", 9)!=0)
			{
				/* skip non-sequence line */
			...
			free(line);
			if (num_seq>=MAX_NUM_NAMES) {
				fprintf(stderr, "Too many sequences in STOCKHOLM file");
				return 0;
			}
		...
		AlignedSeqs[num_seq] = NULL;
		if (num_seq == 0) {
			fprintf(stderr, "No sequences found in STOCKHOLM file\n");
			return 0;
		}
		n = strlen(AlignedSeqs[0]);
		for (nn=1; nn<num_seq; nn++) {
